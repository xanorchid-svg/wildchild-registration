...
function getWeeksForMonth(year, month) {
  ...
  while (cur.y < year || (cur.y === year && cur.m <= month)) {
    const fri = addDays(cur, 4);
    weeks.push({ monday: cur, friday: fri });
    cur = addDays(cur, 7);
  }
  return weeks;
}
function ymdEqual(a, b) { return a.y===b.y && a.m===b.m && a.d===b.d; }
function ymdLt(a, b) {
...
function discountPct(prevWeeks, newWeeks, hasLocalCode, localBasePct) {
  ...
  if (total >= 18) return null; // flat rate applies
  if (hasLocalCode) {
...
function calcChildTotal(child, selectedDays, prevWeeks, hasLocalCode, localBasePct, siblingDiscount, referralPct) {
  ...
  const weekMap = {};
  days.forEach(k => {
    const ymd = parseLocalKey(k);
    const mon = getMonday(ymd);
    const wk  = localDateKey(mon);
    weekMap[wk] = (weekMap[wk] || 0) + 1;
  });
  const weeks = Object.keys(weekMap).length;
  const totalWeeks = prevWeeks + weeks;
  const useFlat = totalWeeks >= 18;
  const dPct = useFlat ? null : discountPct(prevWeeks, weeks, hasLocalCode, localBasePct);

  let tuition = 0;
  Object.values(weekMap).forEach(cnt => {
    const rate = weeklyRate(program, cnt, useFlat);
    tuition += rate;
  });
...
export default function WildChildRegistration() {
  ...
  const [selectedDays, setSelectedDays] = useState({}); // { childIndex: [dateKey, ...] }
  ...
  async function validateLocalCode() {
    ...
    if (error || !data || !data.active) {
      setLocalCodeValid(false);
    } else {
      setLocalCodeValid(true);
      setLocalCodePct(data.discount_pct || 20);
    }
  }
  ...
  async function submitRegistration(paymentIntentId) {
    ...
    try {
      ...
      // Save each child's registration
      const savedIds = [];
      for (let i = 0; i < children.length; i++) {
        ...
        const { data: reg, error: regErr } = await supabase.from('registrations').insert({
          program_id:            ch.program,
          program_name:          prog?.name || ch.program,
          child_first_name:      ch.firstName,
          child_last_name:       ch.lastName,
          child_dob:             ch.dob,
          child_allergies:       ch.allergies,
          parent_name:           parentInfo.name,
          parent_email:          parentInfo.email,
          parent_phone:          parentInfo.phone,
          selected_days:         selectedDays[i] || [],
          lunch,
          ...
          waiver_media:          waivers.media,
          waiver_excursion:      waivers.excursion,
          ...
          discount_pct:          tot.discPct || 0,
          flat_rate_applied:     tot.useFlat,
          weeks_total:           tot.weeks,
        ...
        // Update cumulative weeks on children table
        if (userId) {
      ...
      // Save waiver to profile if creating account
  ...
  // ── Navigation helpers ──────────────────────────────────────────────────────
  function nextStep() {
    // Skip waiver step if already signed
    if (step === 3) {
      const alreadySigned = user && waivers.liability && waivers.medical && signature;
      if (alreadySigned) { setStep(5); return; }
      setStep(4);
      return;
    }
    setStep(s => s + 1);
  ...
  function updateChild(i, field, value) {
    setChildren(prev => {
      ...
      // Clear program if DOB changes and current program no longer eligible
      if (field === 'dob') {
        const eligible = eligiblePrograms(value);
        if (!eligible.find(p => p.id === next[i].program)) {
          next[i].program = '';
        }
      }
      return next;
  ...
  function toggleDay(childIdx, key) {
    setSelectedDays(prev => {
      ...
      const dow  = dowOf(ymd.y, ymd.m, ymd.d);
      if (dow === 0 || dow === 6) return prev; // no weekends
      const has  = days.includes(key);
      // Check per-week max 5
      if (!has) {
        const mon = localDateKey(getMonday(ymd));
        const wkCount = days.filter(d => {
          const m2 = localDateKey(getMonday(parseLocalKey(d)));
          return m2 === mon;
        }).length;
  ...
  return (
    <div style={{ minHeight: '100vh', background: CREAM, margin: 0, padding: 0 }}>
      ...
      {/* Header */}
      <header style={{
        background: OLIVE_DARK, padding: '0 24px',
        ...
        {step === 0 && (
          <div>
            ...
            <p style={{ color: '#666', marginBottom: 24, fontSize: 14 }}>
              Add up to 5 children. Enter each child's date of birth first — their eligible programmes will appear automatically.
            </p>
                    ...
                    return (
                      <div style={{ marginTop: 16, padding: '14px 16px', background: '#fff8e1',
                        ...
                        {age !== null && age > 12
                          ...
                          : 'Please enter a valid date of birth to see available programmes.'}
                      </div>
                    );
                  }
                  return (
            ...
            {/* Lunch add-on */}
            <div style={{ marginTop: 24, padding: '16px', background: '#fff',
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: '#666', marginTop: 2 }}>
                    Fresh, locally sourced meals prepared daily by our chef.
                  </div>
                </div>
              </label>
            </div>

            <NavButtons onNext={nextStep} nextDisabled={
              children.some(ch => !ch.firstName || !ch.lastName || !ch.dob || !ch.program)
            } />
          </div>
        )}
        ...
        {step === 1 && (
          <div>
            ...
            <p style={{ color: '#666', marginBottom: 16, fontSize: 14 }}>
              Select school days for each child. Minimum 3 days per week, maximum 5 (Monday–Friday only). Pricing is calculated per week.
            </p>
            ...
            <div style={{ marginTop: 24, background: '#fff', border: `1px solid ${CREAM_DARK}`,
              ...
              <div style={{ fontWeight: 700, color: OLIVE_DARK, marginBottom: 10, fontSize: 15 }}>
                Pricing reference
              </div>
              ...
              <p style={{ fontSize: 12, color: '#888', marginTop: 10, marginBottom: 0 }}>
                Volume discounts apply at 4+ cumulative weeks. Local families: use code <strong>nosaralocals</strong> on the next step.
              </p>
            </div>

            <NavButtons onBack={prevStep} onNext={nextStep}
              nextDisabled={children.some((ch, i) => {
                const days = selectedDays[i] || [];
        ...
        {step === 2 && (
          <div>
            ...
            <Field label="Referral code (optional)">
              ...
              {referralValid === true  && <p style={{ color: GREEN,  fontSize: 13, marginTop: 4 }}>✓ Referral applied — 5% off tuition</p>}
              ...
              <div style={{ marginTop: 8, padding: 16, background: '#fff',
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 13, color: '#666', marginTop: 2 }}>
                      Access your enrolment details, schedule, and future re-enrolments.
                    </div>
                  </div>
                </label>
                {createAccount && (
        ...
        {step === 3 && (
          <Elements stripe={stripePromise}>
            <PaymentStep
              ...
              totalWeeksAll={totalWeeksAll}
              paymentPlan={paymentPlan}
              setPaymentPlan={setPaymentPlan}
              installmentAmt={installmentAmt()}
              hasLocalCode={hasLocalCode}
              localCodePct={localCodePct}
        ...
        {step === 4 && (
          <div>
            ...
            <p style={{ fontSize: 14, color: '#666', marginBottom: 20 }}>
              Please read and agree to each section before signing.
            </p>
            ...
            {[
              ...
              { key: 'medical',   title: '2. Medical & Emergency Consent',
                text: 'I authorise Wild Child to seek emergency medical care if I cannot be reached. I consent to examination, diagnosis, treatment, or hospital care deemed necessary by a physician. I understand all medical expenses are my responsibility.' },
            ...
            <NavButtons onBack={() => setStep(3)} onNext={() => {
              // Waiver complete — go directly to submit (reuse payment flow that already ran)
              // Actually submit is called from payment step; waiver just needs to be filled before confirmation
              setStep(5);
            }}
            ...
            {paymentPlan !== 'full' && (
              <div style={{ background: '#fff', border: `1px solid ${CREAM_DARK}`, borderRadius: 8,
                ...
                <div style={{ fontSize: 14, color: '#555' }}>
                  {paymentPlan === 'biweekly' ? 'Bi-weekly' : 'Monthly'} payments of{' '}
                  <strong>${installmentAmt()}</strong>. Your first payment has been processed.
                  Subsequent payments will be arranged with the Wild Child team.
                </div>
              </div>
            )}
            {user && (
...
function ProgramCard({ prog, selected, onSelect }) {
  return (
    <div onClick={onSelect} style={{
      border: `2px solid ${selected ? OLIVE_DARK : CREAM_DARK}`,
      ...
      <p style={{ fontSize: 12, color: OLIVE, marginTop: 6, marginBottom: 0, fontWeight: 600 }}>
        From ${prog.rateStd[3]}/week (3 days)
        {prog.halfDay ? ' · Half day, 3 hrs' : ' · Full day'}
      </p>
    </div>
  );
}
...
// ─── Child calendar ───────────────────────────────────────────────────────────
function ChildCalendar({ child, childIdx, selectedDays, onToggle, calMonth, calYear, onPrevMonth, onNextMonth, today }) {
  ...
  // Group selected by week for summary
  const weekGroups = {};
  selectedDays.forEach(k => {
    const mon = localDateKey(getMonday(parseLocalKey(k)));
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <span style={{ fontWeight: 700, color: OLIVE_DARK, fontSize: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 4, marginBottom: 6 }}>
        {DAY_ABBR.map(d => (
          <div key={d} style={{ textAlign: 'center', fontSize: 12, fontWeight: 700,
            color: '#888', paddingBottom: 4 }}>{d}</div>
        ))}
      </div>
      ...
      {sortedWeeks.length > 0 && (
        <div style={{ marginTop: 16, padding: 14, background: '#fff',
          border: `1px solid ${CREAM_DARK}`, borderRadius: 8, fontSize: 13 }}>
          <div style={{ fontWeight: 700, color: OLIVE_DARK, marginBottom: 8 }}>
            {child.firstName}'s schedule summary
          </div>
          {sortedWeeks.map(mon => {
            ...
            return (
              <div key={mon} style={{ display: 'flex', justifyContent: 'space-between',
                ...
                <span style={{ color: '#555' }}>
                  {cnt} day{cnt!==1?'s':''} · week of {MONTH_NAMES[ymd.m-1]} {ymd.d}
                </span>
...
function PaymentStep({ childTotals, children, selectedDays, lunch, lunchTotal,
  ...
  async function handlePay() {
    ...
    try {
      const { data, error: fnErr } = await supabase.functions.invoke('create-payment-intent', {
        body: { amount: Math.round(installmentAmt * 100), currency: 'usd' }
      });
  ...
  return (
    <div>
      ...
      <div style={{ background: '#fff', border: `1px solid ${CREAM_DARK}`, borderRadius: 8,
          ...
          return (
            <div key={i} style={{ borderTop: `1px solid ${CREAM_DARK}`, padding: '8px 0' }}>
              ...
              <div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>
                ...
                {tot.useFlat ? '· flat rate applied' : tot.discPct ? `· ${tot.discPct}% discount` : ''}
                {i > 0 ? ' · sibling 10%' : ''}
                {referralPct ? ` · referral ${referralPct}%` : ''}
              </div>
            </div>
          );
        })}
        {lunch && (
          <div style={{ display: 'flex', justifyContent: 'space-between',
            ...
            <span>Organic snack & lunch</span>
            <span>${lunchTotal.toFixed(2)}</span>
          </div>
        )}
        <div style={{ display: 'flex', justifyContent: 'space-between',
          borderTop: `2px solid ${OLIVE_DARK}`, paddingTop: 10, marginTop: 4,
          fontWeight: 700, fontSize: 16, color: OLIVE_DARK }}>
          <span>Total</span>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {[
            ...
            <label key={opt.id} style={{ display: 'flex', alignItems: 'center', gap: 12,
              padding: '10px 14px', marginBottom: 8, background: paymentPlan===opt.id ? '#f0f4e8' : '#fff',
              border: `1.5px solid ${paymentPlan===opt.id ? OLIVE_DARK : CREAM_DARK}`,
          ...
          {paymentPlan !== 'full' && (
            <p style={{ fontSize: 13, color: '#888', marginTop: 4 }}>
              First instalment of <strong>${installmentAmt.toFixed(2)}</strong> charged today. Subsequent payments arranged with the Wild Child team.
            </p>
          )}
        </div>
      )}
      ...
      {(payErr || error) && (
        <p style={{ color: '#c00', fontSize: 14, marginBottom: 12 }}>{payErr || error}</p>
      )}
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
        ...
        <button onClick={handlePay} disabled={paying || loading} style={{
          ...
          padding: '16px 24px', fontSize: 16, fontWeight: 700, cursor: paying || loading ? 'default' : 'pointer',
        }}>
          {paying || loading
            ? 'Processing…'
            : `Pay $${installmentAmt.toFixed(2)}${paymentPlan !== 'full' ? ' (first instalment)' : ''}`}
        </button>
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
      ...
      {onNext && (
        <button onClick={onNext} disabled={nextDisabled} style={{
          ...
          color: nextDisabled ? '#999' : CREAM,
          border: 'none', borderRadius: 6, padding: '16px 24px',
          fontSize: 16, fontWeight: 700, cursor: nextDisabled ? 'default' : 'pointer',
        }}>{nextLabel}</button>
      )}
    </div>
  );
...
const inputStyle = {
  ...
  border: `1px solid ${CREAM_DARK}`, background: '#fff', color: '#333',
  boxSizing: 'border-box', outline: 'none', fontFamily: 'inherit',
};
const labelStyle = {
  display: 'block', fontSize: 13, fontWeight: 700, color: OLIVE_DARK,
  marginBottom: 6, letterSpacing: '0.03em',
};
const backBtnStyle = {
  background: CREAM_DARK, color: OLIVE_DARK, border: 'none', borderRadius: 6,
  padding: '16px 20px', fontSize: 15, fontWeight: 600, cursor: 'pointer',
};
const arrowBtn = {
  background: 'none', border: 'none', fontSize: 22, cursor: 'pointer',
  color: OLIVE_DARK, padding: '4px 12px', borderRadius: 4,
};
