...
export default function WildChildRegistration() {
  ...
  // selectedDays: Set of individual day keys (ISO date strings)
  ...
  const grandTotal = subtotalTuition + subtotalLunch;
  ...
  const next = async () => {
    ...
    if (step === 4) {
      ...
      const { error } = await supabase.from("registrations").insert({
        ...
        child_first_name: child.fn,
        child_last_name: child.ln,
        child_dob: child.dob,
        ...
        lunch,
        subtotal_tuition: subtotalTuition,
        subtotal_lunch: subtotalLunch,
        grand_total: grandTotal,
        waiver_liability: w.liab,
        waiver_medical: w.med,
        waiver_media: w.mediaY ? "yes" : w.mediaN ? "no" : null,
        waiver_excursion: w.excY ? "yes" : w.excN ? "no" : null,
        waiver_signature: sig,
  ...
  return (
    <div style={{ fontFamily:"Georgia,serif", background:CREAM, minHeight:"100vh", color:TEXT_DARK }}>
      ...
      {/* Header */}
      <div style={{ background:TEAL, overflow:"hidden", position:"relative", height:"120px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"flex-end" }}>
        <img src={logo} alt="Wild Child Nosara" style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%, -50%)", width:"100%", maxWidth:"700px", objectFit:"cover", objectPosition:"center" }} />
        <p style={{ fontSize:"11px", letterSpacing:"2px", color:"rgba(255,255,255,0.85)", textTransform:"uppercase", margin:"0 0 10px", position:"relative", zIndex:1 }}>Enrollment Registration</p>
      </div>
      ...
      <div style={{ maxWidth:"600px", margin:"0 auto", padding:"28px 16px 80px" }}>
        ...
        {step===2 && <>
          ...
          <p style={{ fontSize:"14px", color:TEXT_MID, marginBottom:"24px", lineHeight:1.7 }}>
          ...
          <div onClick={()=>setLunch(!lunch)}
            style={{ background:lunch?GREEN:"#fff", border:`1.5px solid ${lunch?GREEN:CREAM_DARK}`, borderRadius:"10px", padding:"14px 16px", cursor:"pointer", display:"flex", alignItems:"center", gap:"14px", marginBottom:"28px", transition:"all .2s" }}>
            <span style={{ fontSize:"22px" }}>🥗</span>
            <div style={{ flex:1 }}>
              <p style={{ fontSize:"14px", color:lunch?"#fff":TEXT_DARK, marginBottom:"2px" }}>Add Organic Snack & Lunch</p>
              <p style={{ fontSize:"12px", color:lunch?"rgba(255,255,255,0.75)":TEXT_LIGHT, lineHeight:1.4, margin:0 }}>All organic, locally sourced, and made with love. $10/day</p>
            </div>
                      ...
                      return (
                        <div key={offset} onClick={()=>!isPast&&!isBlocked&&toggleDay(d)}
                          style={{ textAlign:"center", padding:"8px 2px", borderRadius:"8px", transition:"all .15s",
                            background: isSel ? TEAL : (inMonth ? CREAM : CREAM_DARK),
                            ...
                            border: isSel ? `1.5px solid ${TEAL_DARK}` : "1.5px solid transparent",
                          }}>
          ...
          {weekEntries.length > 0 && (
            <div style={{ background:"#fff", border:`1px solid ${CREAM_DARK}`, borderRadius:"10px", padding:"16px", marginTop:"16px" }}>
                ...
                return (
                  <div key={weekKey(wk.monday)} style={{ borderBottom:`1px solid ${CREAM_DARK}`, paddingBottom:"10px", marginBottom:"10px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:"13px", color:TEXT_DARK, marginBottom:"3px" }}>
                      ...
                      <span style={{ color:TEAL }}>${p + lunchCost}</span>
                    </div>
              ...
              {lunch && subtotalLunch > 0 && (
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:"13px", color:TEXT_MID, paddingBottom:"8px", borderBottom:`1px solid ${CREAM_DARK}` }}>
                  <span>Organic Snack & Lunch ({selectedDays.size} days × $10)</span>
                  <span>${subtotalLunch}</span>
                </div>
              )}
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:"16px", paddingTop:"10px", color:TEXT_DARK }}>
                <span>{weekEntries.length} week{weekEntries.length>1?"s":""} total</span>
                <span style={{ color:TEAL }}>${grandTotal}</span>
              </div>
            </div>
          )}
        </>}
        ...
        {step===3 && <>
          ...
          <div style={{ background:"#fff", border:`1px solid ${CREAM_DARK}`, borderRadius:"10px", padding:"16px", marginBottom:"22px" }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:"17px", paddingTop:"10px", color:TEXT_DARK }}>
              <span>Total Due</span><span style={{ color:TEAL }}>${grandTotal}</span>
            </div>
          </div>
          <div style={{ background:TEAL_LIGHT, border:`1px solid ${TEAL_MID}`, borderRadius:"8px", padding:"11px 14px", marginBottom:"20px", display:"flex", gap:"9px", alignItems:"center" }}>
            <span>🔒</span>
            <p style={{ fontSize:"12px", color:TEAL_DARK, margin:0, lineHeight:1.5 }}>Demo mode — connects to Stripe in production. Enter any 16-digit number to continue.</p>
          </div>
        ...
        {step===5 && (
          <div style={{ textAlign:"center", padding:"16px 0" }}>
            <div style={{ width:"68px", height:"68px", borderRadius:"50%", background:TEAL, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 20px", fontSize:"30px" }}>🌿</div>
            ...
            <p style={{ fontSize:"14px", color:TEXT_MID, maxWidth:"420px", margin:"0 auto 24px", lineHeight:1.6 }}>
              {child.fn||"Your child"} is enrolled in {sp?.name||"our program"}. We're so excited to welcome your family to Wild Child Nosara.
            </p>
            <div style={{ background:"#fff", border:`1px solid ${CREAM_DARK}`, borderRadius:"10px", padding:"18px", maxWidth:"400px", margin:"0 auto 22px", textAlign:"left" }}>
              ...
              {[
                ["Child", `${child.fn} ${child.ln}`.trim()||"—"],
                ["Program", sp?.name||"—"],
                ["Weeks enrolled", `${weekEntries.length} week${weekEntries.length!==1?"s":""}`],
                ["Days selected", `${selectedDays.size} day${selectedDays.size!==1?"s":""}`],
                lunch?["Lunch","Organic Snack & Lunch included"]:null,
                ["Confirmation sent to", parent.email||"your email"],
              ].filter(Boolean).map(([k,v],i,arr)=>(
                <div key={k} style={{ display:"flex", justifyContent:"space-between", fontSize:"13px", padding:"5px 0", borderBottom:i<arr.length-1?`1px solid ${CREAM_DARK}`:"none", color:TEXT_MID }}>
                  <span>{k}</span><span style={{ color:TEXT_DARK }}>{v}</span>
                </div>
              ))}
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:"16px", paddingTop:"10px", color:TEXT_DARK }}>
                <span>Total Paid</span><span style={{ color:TEAL }}>${grandTotal}</span>
              </div>
            </div>
            <div style={{ background:TEAL_LIGHT, border:`1px solid ${TEAL_MID}`, borderRadius:"10px", padding:"14px 18px", maxWidth:"400px", margin:"0 auto 20px", textAlign:"left" }}>
              <p style={{ fontSize:"13px", color:TEAL_DARK, marginBottom:"5px", fontWeight:"bold" }}>What happens next</p>
              <p style={{ fontSize:"13px", color:TEAL_DARK, lineHeight:1.6, margin:0 }}>
                A confirmation email has been sent to {parent.email||"you"}. Our team at info@dandelionwildschooling.com has been notified. We'll be in touch with your welcome packet. Pura vida! 🌺
              </p>
            </div>
            <p style={{ fontSize:"12px", color:TEXT_LIGHT }}>Questions? <a href="mailto:info@dandelionwildschooling.com" style={{ color:TEAL }}>info@dandelionwildschooling.com</a></p>
          </div>
        )}
